...
export function QuickLogModal({ onClose }: QuickLogModalProps) {
 ...
 <button
 onClick={onClose}
 className="text-on-surface-variant hover:text-[#EF4444] transition-colors p-1"
 >
 ...
 <p className="font-headline-lg-mobile text-on-background">+{xpPreview} XP Earned!</p>
 <p className="text-on-surface-variant text-sm mt-1 font-label-mono">Activity logged successfully</p>
 </div>
 ) : (
 <form onSubmit={handleSubmit} className="p-6 space-y-5">
 {/* Activity Type */}
 <div>
 <label className="block font-label-mono text-xs text-on-surface-variant uppercase mb-1.5">
 Activity Type
 </label>
 <select
 value={type}
 ...
 <span className="font-label-mono text-sm text-on-surface-variant">XP Preview</span>
 <span className="font-stat-lg text-primary">+{xpPreview} XP</span>
 </div>

 {/* Actions */}
 <div className="flex gap-3 pt-2 border-t border-outline">
 <button
 type="button"
 onClick={onClose}
 className="flex-1 py-2 border border-outline text-on-surface-variant rounded font-label-mono text-sm hover:bg-background transition-colors"
 >
 Cancel
 </button>
 <button
