...
function SubjectCard({ subject, onToggle }: { subject: Subject; onToggle: (id: string) => void }) {
  ...
  return (
    <article className="bg-white border border-[#E5E5E5] shadow-panel rounded-xl p-sm flex flex-col gap-3 relative overflow-hidden group hover:border-opacity-60 transition-colors">
      {/* Hover top accent */}
      <div
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          <div
            className="w-10 h-10 rounded-lg flex items-center justify-center border border-[#E5E5E5]"
          ...
          <h3 className="font-bold text-[#1A1A1A] text-base leading-tight">{subject.name}</h3>
        </div>
        <Link
          href={`/subjects/${subject.id}`}
          className="text-[#E5E5E5] group-hover:text-[#6B7280] transition-colors"
        >
      ...
      <div className="flex gap-2 flex-wrap font-label-mono text-xs">
        ...
        <span className="bg-[#F9FAFB] text-[#6B7280] px-2 py-1 rounded border border-[#E5E5E5] flex items-center gap-1">
      ...
      {/* Progress */}
      <div className="mt-auto pt-3 border-t border-[#E5E5E5] flex justify-between items-end">
        <div className="flex flex-col gap-1 flex-1 mr-4">
          <div className="flex justify-between text-xs font-label-mono text-[#6B7280]">
      ...
      <div className="glass-panel rounded-xl p-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#F9FAFB] border border-[#E5E5E5] rounded pl-10 pr-4 py-2 text-sm font-label-mono text-[#1A1A1A] focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-[#6B7280]/50"
          />
        </div>
      </div>
